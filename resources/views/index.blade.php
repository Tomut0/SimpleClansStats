                    ...
                    <table class="table table-success table-striped w-100">
                        ...
                        @foreach($players->getTopPlayersByKDR() as $id => $player)
                            <tr>
                                ...
                                <td><img src="https://mc-heads.net/avatar/{{ $player['Name'] }}/20/"
                    ...
                    <table class="table table-warning table-striped w-100">
                        ...
                        @foreach($players->getLastPlayersKills() as $id => $player)
                            <tr>
                                <th scope="row">{{ $id+1 }}</th>
                                <td><span
                                        class="badge bg-dark .rounded-pill">{!! $player->attacker_colored_tag !!}</span> {{ $player->attacker }}
                                </td>
                                <td><span
                                        class="badge bg-dark .rounded-pill">{!! $player->victim_colored_tag !!}</span> {{ $player->victim }}
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
